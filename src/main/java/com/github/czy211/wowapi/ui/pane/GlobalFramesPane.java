                ...
                if (file.isDirectory()) {
                    ...
                    if (filename.endsWith(".xml")) {
                        try {
                            ...
                            Elements elements = Jsoup.parse(in, "UTF-8", "", Parser.xmlParser()).select("[name]"
                                    + ":not([name^=$],[virtual=true],[intrinsic=true],[parentKey],Binding,Font,"
                                    + "Attribute)");
                ...
                if (file.isDirectory()) { // 解析文件夹
                    ...
                    try {
                        if (filename.endsWith(".lua")) { // 解析 lua 文件
                            ...
                            BufferedInputStream in = new BufferedInputStream(new FileInputStream(file));
                            Elements elements = Jsoup.parse(in, "UTF-8", "", Parser.xmlParser()).select("[name]"
                                    + ":not([name^=$],Font,FontFamily,Texture)[virtual=true],[intrinsic=true]");
                            for (Element element : elements) {
                                if (Thread.currentThread().isInterrupted()) {
                                    canceled();
                                    return;
